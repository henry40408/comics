...
- **`auth/`** — form-login authentication. `auth_middleware_fn` guards content routes; `config.rs` models credentials as `AuthConfig::{Some, None}`; `ratelimit.rs` throttles `POST /login` and `trusted_proxies.rs` decides whose `X-Forwarded-For` may name the client it throttles.
