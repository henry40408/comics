...
- **`secret.rs`** — `Secret`, the single configured value (`COMICS_SECRET`, 128 hex characters). Parsed by `clap` through `FromStr`, so a malformed value fails during argument parsing rather than at startup, and `Debug` is redacted so the value cannot ride along when `Opts` is formatted for a log. Two values are derived from it, each behind its own domain separator: `session_key()` (SHA-512, which is exactly the 64 bytes `cookie::Key` wants) and `id_seed()` (SHA-256, truncated to 8 bytes). The separation is not decorative — book IDs are `xxh3(seed, title)` and appear in URLs, xxh3 is not cryptographic, so the seed must be treated as public; deriving it through a hash keeps a recovered seed from leaking any cookie-signing material.
